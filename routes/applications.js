...
function reorderFields(applicantDetails) {
    ...
    const reordered = { ...applicantDetails }; // Preserve all fields
    // Reorder only if needed for specific display logic
    return reordered;
}
...
router.post('/submit', upload.fields([
    ...
    try {
        ...
        // Preserve all submitted fields without reordering
        ...
        for (const field in files) {
            if (field === 'passport_photo' && files[field].length > 0) photoPath = files[field][0].path;
        ...
        const formattedDetails = Object.fromEntries(Object.entries(applicantDetails).map(([key, value]) => {
            const { key: formattedKey, value: formattedValue } = formatField(key, value);
        ...
        const mailOptionsAdminTrustee = { from: process.env.EMAIL_USER, subject: 'New Scholarship Form Received - ID: ' + trackingId, html: adminTrusteeHtmlTable, attachments: [{ filename: `app_${trackingId}.docx`, content: buffer }] };
        const mailOptions = { from: process.env.EMAIL_USER, subject: `AUYPCT Application - ID: ${trackingId}`, html: applicantHtmlTable, attachments: [{ filename: `app_${trackingId}.docx`, content: buffer }] };

        await Promise.all([
            transporter.sendMail({ ...mailOptions, to: applicantDetails.email_id }),
            transporter.sendMail({ ...mailOptionsAdminTrustee, to: 'yuvaneshr2002@gmail.com' }),
            transporter.sendMail({ ...mailOptionsAdminTrustee, to: 'rdmvyfamily@gmail.com' })
        ]);
    ...
    try {
        ...
        const overview = applications.map(app => {
            ...
            return {
                ...
                photo: app.photoPath ? `/uploads/${path.basename(app.photoPath)}` : null,
                // Include a subset of key details for overview
                keyDetails: {
    ...
    try {
        ...
        const formattedDetails = Object.fromEntries(
            Object.entries(applicantDetails).map(([key, value]) => {
                try {
                    const { key: formattedKey, value: formattedValue } = formatField(key, value);
                    if (key === 'dob' || key === 'application_date') {
                        formattedValue = formatDate(value) || 'N/A';
                    }
                    return [formattedKey, formattedValue || 'N/A'];
                } catch (err) {
                    console.error(`Error formatting ${key} for ${trackingId}:`, err);
                    return [key, 'N/A'];
                }
            })
        );
        ...
        const processedDocuments = documents.map(doc => {
            try {
                ...
                return { name: doc.name, path: `/uploads/${path.basename(doc.path)}` };
            } catch (err) {
        ...
        const photo = application.photoPath ? `/uploads/${path.basename(application.photoPath)}` : null;
        if (photo && !fs.existsSync(application.photoPath)) {
            console.warn(`Photo file not found for ${trackingId}: ${application.photoPath}`);
        }
        ...
        const details = {
            ...
            rawApplicantDetails: applicantDetails // Include raw data for full visibility
        };
    ...
    try {
        ...
        const formattedDetails = Object.fromEntries(
            Object.entries(applicantDetails).map(([key, value]) => {
                try {
                    const { key: formattedKey, value: formattedValue } = formatField(key, value);
                    if (key === 'dob') formattedValue = formatDate(value) || 'N/A';
                    return [formattedKey, formattedValue || 'N/A'];
                } catch (err) {
                    console.error(`Error formatting ${key} for ${req.params.trackingId}:`, err);
                    return [key, 'N/A'];
                }
            })
        );
...
router.put('/application/:trackingId/status', async (req, res) => {
    try {
        ...
        if (!status || !['Submitted', 'Under Review', 'Eligible', 'Rejected', 'Funds Transferred'].includes(status)) {
        ...
        await Application.findOneAndUpdate({ trackingId: req.params.trackingId }, { status, bankDetails, reviewedBy });

        if (status !== app.status && ['Under Review', 'Eligible', 'Rejected', 'Funds Transferred'].includes(status)) {
            const applicantDetails = app.applicantDetails || {};
            const formattedDetails = Object.fromEntries(
                Object.entries(applicantDetails).map(([key, value]) => {
                    const { key: formattedKey, value: formattedValue } = formatField(key, value);
            ...
            const message = `Your AUYPCT application (ID: ${app.trackingId}) status has been updated to ${status}.`;
            const currentDateTime = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' });
            const htmlAlert = `
                <html><head><style>body { font-family: Arial, sans-serif; color: #333; } .container { max-width: 600px; margin: 0 auto; padding: 20px; } h2 { color: #1e3c72; } table { border-collapse: collapse; width: 100%; margin: 20px 0; } th, td { border: 1px solid #ddd; padding: 12px; text-align: left; } th { background-color: #f2f2f2; } .footer { margin-top: 20px; font-size: 0.9em; color: #666; }</style></head><body><div class="container"><h2>Status Update</h2><p>${sanitizeHtml(message)}</p><table><tr><th>Field</th><th>Value</th></tr>${Object.entries(formattedDetails).map(([key, value]) => `<tr><td>${sanitizeHtml(key)}</td><td>${sanitizeHtml(value)}</td></tr>`).join('')}</table><div class="footer"><p>Contact us at: auytrust@gmail.com</p><p>Update Date: ${currentDateTime}</p></div></div></body></html>
            `;

            await transporter.sendMail({
                from: process.env.EMAIL_USER,
                to: app.applicantDetails.email_id,
                subject: `AUYPCT Application Status Update - ${app.trackingId}`,
                html: htmlAlert
            });
        }

        console.log(`Status updated for ${req.params.trackingId} to ${status}`);
